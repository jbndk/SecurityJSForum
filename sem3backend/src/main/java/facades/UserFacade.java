...
public class UserFacade {
    ...
    public String newUser(UserDTO userDTO) {
        ...
        String returnString = newuser.getUserName();
        ...
        if (userDTO.getUserName().length() == 0 || (userDTO.getUserPass().length() == 0)) {
            return returnString = "One or both values are missing";
        } 
        
        try {
            if (em.find(User.class, newuser.getUserName()).getUserName().equals(newuser.getUserName())) {
            return returnString = "User already exists";
        }
        } catch (Exception e) {
        }
            ...
            try {
                System.out.println("Linje 104"+newuser.getUserName());
                em.getTransaction().begin();
